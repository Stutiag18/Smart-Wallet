...
const api = axios.create({
  baseURL: import.meta.env.VITE_API_BASE_URL || 'https://smart-wallet-backend-o141.onrender.com/api'
});
...
export const panService = {
  submitPan: async (userId, panName, panNumber, dob) => {
    const response = await api.post('/v1/pan-screen/details', { userId, panName, panNumber, dob });
    return response.data;
  },
  getStatus: async (userId) => {
    const response = await api.get(`/v1/pan-screen/status/user/${userId}`);
    return response.data;
  }
};
